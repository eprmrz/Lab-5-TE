...
read.csv("data_raw/teacher_evals.csv")
```
...
t_eval <- read.csv("data_raw/teacher_evals.csv")
glimpse(t_eval)
```
> There are 8,015 rows and 22 columns in our dataset. I also notice that some of the columns have capital letters in the variable names, which means we will have to use the janitor package to make them all lowercase and easier to work with. Additionally, we have various character and integer, and double types of data in our data set. The class duration column shows only the number 0 in the observations, which stands out to me which means that we may have to replace the zeros with NAs.
...
print(t_eval)
```
> The unit of observation is identified by the variable type, such as <chr>, <dbl>, and <int>.
...
#Save the data as `teacher_evals_clean.csv` in the `data-clean` folder
#write.csv(teacher_evals_clean, "C:/Users/efrai/Documents/STAT_210_Spring_2025/Labs/Original/Lab-5-efrain-ramirez/Lab-5-TE/data_clean/teacher_evals_clean.csv")
```

**6. How many unique instructors and unique courses are present in the cleaned dataset?**
> There are 308 unique instructors and 1053 unique courses present in the cleaned dataset.

```{r}
#| label: unique-courses
# code chunk for Q5
teacher_evals_clean %>% 
  distinct(teacher_id) #308 unique instructors are present

teacher_evals_clean %>% 
  distinct(course_id) #1053 unique courses are present
...
>I honestly had the toughest trying to figure out how to find the NAs in the dateset through the use of dplyr functions that I had to move on to the other questions. I spent about 3 hours trying to figure this one out.

```{r}
#| label: uncovering-missing-values
# code chunk for Q6

# I honestly had the hardest time being able to find this one.
```
...
> The demographics of the instructors in this study were: academic_degree; no_dgr, ma, dr, and prof. Meaning that no_dgr was no degree, ma was Masters, dr was a Doctorate, and prof, meant that they were a professor, which was strange to me that it would be under the list of academic degrees. As for seniority, this had to do with the number of years that instructors had been instructors for at the university. Lastly, the variable of sex was determined by self-reported answer of either Male or Female by the respondent.
...
unique(teacher_evals_clean$seniority)

unique(teacher_evals_clean$sex)
```
...
> The amount of teacher-course combinations were a total of 882 that asked all nine questions.
...
teacher_evals_clean %>% 
  select(course_id, teacher_id, question_no) %>% 
  filter(question_no == 909) 
...
> The instructor with teacher_id: 54655 had the highest with an average of 4.22 on the SET_score_avg, while the instructor with teacher_id: 100132 had the lowest average with a score of 1.0 on the SET_score_avg. 

```{r}
#| label: question-1-high-low
# code chunk for Q9
teacher_evals_clean %>% 
  select(teacher_id, question_no, SET_score_avg ) %>% 
...
> The instructor with one year of experience that had the highest average percentage of students failing their current semester across all their courses is teacher_id: 104362, while the instructor with one year of experience that had the lowest average percentage of students failing their current semester across all their courses is teacher_id: 98650, but also 104362.

```{r}
#| label: one-year-experience-failing-students
# code chunk for Q10
teacher_evals_clean %>% 
  select(teacher_id, seniority, percent_failed_cur) %>% 
...
> The female instructors with either a doctorate or a professional degree that had the highest average percent of students responding to the evaluation across all their courses were instructors with the teacher_id : 104362 and 57131. The female instructors with either a doctorate or a professional degree that had the lowest average percent of students responding to the evaluation across all their courses were instructors with the teacher_id: 3485, and 59338.
